...
import { ArrowLeft, GraduationCap, IdCard, MapPin, Receipt, Sparkles, Ticket, User, Users } from 'lucide-react'
...
export const Route = createFileRoute('/souscription/recapitulatif')({
  validateSearch: z.object({ code: z.string().optional() }),
  component: RecapitulatifStep,
})

function useAbonnementParCode(code: string | undefined) {
  const [abo, setAbo] = React.useState<TypeAbonnement | null>(null)
  const [chargement, setChargement] = React.useState(false)
  React.useEffect(() => {
    if (!code) return
    setChargement(true)
    getAbonnements()
      .then((list) => setAbo(list.find((a) => a.code === code) ?? null))
      .catch(() => setAbo(null))
      .finally(() => setChargement(false))
  }, [code])
  return { abo, chargement }
}
...
function RecapitulatifStep() {
  ...
  const zonesAbonnement = aboDirecte ? aboDirecte.zones.join(' · ') : (resultat ? selectionnerAbonnement(resultat, wizard.abonnementSelectionneId).abonnement.zones.replace('Zones ', '') : '')
  ...
  async function sauvegarderEtQuitter() {
    ...
    try {
      const reponse = await creerDossier(construirePayloadDossier(wizard, codeAbonnement, undefined, true))
  ...
  return (
    <main className="mx-auto flex max-w-2xl flex-col gap-6 py-8">
      ...
      <ProgressBar etapeCourante={wizard.pourQui === 'TIERS' ? 4 : 3} totalEtapes={wizard.pourQui === 'TIERS' ? 5 : 4} />
      <div className="flex items-center justify-between">
        <h1 className="font-heading text-2xl font-bold tracking-tight text-dark">{m.wizard_recap_title()}</h1>
        {!isDirectPath && (
          <button type="button" className="text-sm font-medium tracking-wide text-primary uppercase underline" onClick={() => navigate({ to: '/recommandation/pour-qui' })}>
      ...
      <dl className="ticket-card">
        <div className="ticket-card__band">
          <span className="font-mono text-xs font-semibold tracking-[0.15em] uppercase">{m.wizard_recap_profile_label()}</span>
        </div>
        <div className="ticket-card__body flex flex-col gap-3">
          <Champ icon={Users} titre={m.wizard_recap_demand_for()} valeur={wizard.pourQui === 'TIERS' ? m.wizard_recap_third_party() : m.wizard_recap_myself()} />
          {wizard.pourQui === 'TIERS' ? <Champ icon={Users} titre={m.wizard_recap_beneficiary()} valeur={`${wizard.infosTiers.prenom} ${wizard.infosTiers.nom}`.trim()} /> : null}
          {situationLabel ? <Champ icon={User} titre={m.wizard_recap_situation()} valeur={situationLabel} /> : null}
          <Champ icon={MapPin} titre={m.wizard_recap_zones()} valeur={zonesAbonnement} />
          <Champ icon={Ticket} titre={m.wizard_recap_subscription()} valeur={nomAbonnement} />
          <Champ icon={IdCard} titre={m.wizard_recap_id_card()} valeur={wizard.pieceIdentiteNomFichier ?? 'Non fournie'} />
          {wizard.situation === 'ETUDIANT' ? <Champ icon={GraduationCap} titre={m.wizard_recap_school_cert()} valeur={wizard.justificatifEtudiantNomFichier ?? 'Non fourni'} /> : null}
          {wizard.situation === 'ETUDIANT' ? <Champ icon={Receipt} titre={m.wizard_recap_boursier()} valeur={wizard.boursier ? m.wizard_recap_boursier_yes() : m.wizard_recap_boursier_no()} /> : null}
          {wizard.situation === 'ETUDIANT' && wizard.boursier ? <Champ icon={Receipt} titre={m.wizard_recap_scholarship_notif()} valeur={wizard.notificationBourseNomFichier ?? 'Non fournie'} /> : null}
          <Champ icon={Sparkles} titre={m.wizard_recap_preverif()} valeur={wizard.verificationIA === 'EFFECTUEE' ? m.wizard_recap_preverif_done() : m.wizard_recap_preverif_none()} />
        </div>
      </dl>

      <div className="flex flex-col gap-3">
        <Button onClick={() => navigate({ to: '/souscription/paiement', search: code ? { code } : {} })} disabled={!piecesCompletes}>
        ...
        {!piecesCompletes ? (
          <p className="text-center text-xs text-gray-700">
            ...
            <button type="button" className="text-primary underline" onClick={() => navigate({ to: '/souscription/pieces', search: code ? { code } : {} })}>
        ...
        {erreur ? (
          <div className="rounded-lg bg-danger-light/15 border border-danger-light/40 px-3 py-2 text-sm text-danger">
            {erreur.type === 'non-authentifie' ? (
              <>{m.wizard_recap_not_connected()}{' '}<Link to="/login" className="font-medium underline">{m.wizard_not_connected_login()}</Link></>
            ) : erreur.message}
          </div>
        ) : null}
...
function Champ({ icon: Icon, titre, valeur }: { icon: typeof User; titre: string; valeur: string }) {
  return (
    <div className="flex items-center justify-between gap-3 border-b-2 border-dashed border-gray-300 pb-2 last:border-0 last:pb-0">
      ...
      <dd className="max-w-[10rem] truncate font-mono font-semibold text-dark" title={valeur}>{valeur}</dd>
    </div>
  )
}
